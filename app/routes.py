...
def signup():
    form = SignUpForm()
    # if the form is submited and all data is valid
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(username = username).first() #Проверяет какой из существующих пользователей заходит, через базу данных
        if user is not None and user.check_password(passowrd): #Если юзер существует и пароль соответствует, то выполнить вход
            login_user(user) #выполняет вход
